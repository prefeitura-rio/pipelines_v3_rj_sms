...
with Flow(
  ...
  with case(RENAME_FLOW, True):
    rename_current_flow_run(
      environment=ENVIRONMENT, target_date=target_date, endpoint=ENDPOINT
    )

  endpoint_params, table_names = generate_endpoint_params(
    target_date=target_date, environment=ENVIRONMENT, table_id_prefix=TABLE_ID_PREFIX
  )
  ...
  extracted_data = get_property_from_dict.map(
    dict=extraction_results, key=unmapped("data")
  )
  logs = get_property_from_dict.map(dict=extraction_results, key=unmapped("logs"))
  ...
  send_email_notification(
    logs=logs, endpoint=ENDPOINT, environment=ENVIRONMENT, target_date=target_date
  )
