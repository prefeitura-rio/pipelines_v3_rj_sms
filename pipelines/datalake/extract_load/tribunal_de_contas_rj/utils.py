...
def split_case_number(case_num: str) -> tuple:
  ...
  num = m.group("num")
  year = m.group("year")
  assert len(year) == 4, f"Ano '{year}' tem tamanho {len(year)}; esperado 4"
  return (sec, num, year)
...
def send_request(
  ...
  log(f"Enviando requisição {method} esperando resposta '{expected_type}': {url}")
  ...
  if ct not in ALLOWED_CONTENT_TYPES:
    log(f"Recebido Content-Type proibido '{ct}' para URL '{url}'", level="warning")
    return (res.url, None)

  if expected_type == "json":
    if ct != "application/json":
      log(f"Esperado Content-Type 'application/json'; recebido '{ct}'", level="warning")
    log(f"Recebida resposta de tamanho {len(res.text)}")
    return (res.url, res.json())

  if expected_type == "html":
    if ct != "text/html":
      log(f"Esperado Content-Type 'text/html'; recebido '{ct}'", level="warning")
    log(f"Recebida resposta de tamanho {len(res.text)}")
...
def get_table_rows_from_h5(h5: BeautifulSoup):
  ...
  table_wrapper: BeautifulSoup = h5.parent.parent.find_next_sibling(
    "div", attrs={"class", "row"}
  )
