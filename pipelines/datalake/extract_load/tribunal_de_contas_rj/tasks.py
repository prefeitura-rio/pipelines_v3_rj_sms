...
def fetch_case_page(case_num: str, env: Optional[str] = None) -> tuple:
  ...
  if title.endswith("resultado de pesquisa a processos"):
    log("Caso possui referências; tentando requisitar conteúdo do caso principal")
    ...
    if a is None or not a:
      ...
      raise RuntimeError("Não foi possível encontrar o ID do caso principal")
  ...
  if not m:
    log(url, level="warning")
    raise RuntimeError("Não foi possível encontrar o ID do caso principal")
  ctid = m.group("ctid")
  log(f"ID detectado: ctid='{ctid}'")

  case_obj = {
    "_extracted_at": now(),  # Salva data/hora em que a página foi extraída
    "_ctid": ctid,
...
def get_latest_vote(ctid: str):
  if not ctid:
    log("Nenhum ctid recebido; ignorando", level="warning")
    return None
  ...
  if div:
    ...
    if not_allowed_text in div.get_text():
      log("Votos indisponíveis nesse caso (interesse pessoal)", level="warning")
...
def scrape_case_info_from_page(case_tuple: tuple) -> dict:
  assert len(case_tuple) == 2, f"Esperados 2 parâmetros; recebido(s) {len(case_tuple)}"
  ...
  def get_information_from_label(root: BeautifulSoup, label_for: str):
    ...
    if p is None or not p:
      log(
        f"Informação não encontrada no caso (for='{label_for}'); será nulo",
        level="warning",
      )
      return None
  ...
  case_obj["detalhes"] = get_information_from_label(
    page, "Processo_Detalhamento_ObjetoDetalhado"
  )
  ...
  rows = get_rows_from_text(
    page, r"decisões do processo", throw="Não foi possível encontrar decisões do processo"
  )
  ruling = get_row_contents(rows[0])
  # Deve ser algo como ['01/01/2000', 'Contas Regulares']
  if len(ruling) <= 1:
    log(ruling, level="error")
    raise RuntimeError("Falha ao obter decisão mais recente na página!")
  ...
  for row in rows:
    ...
    if part_name == "parte":
      ...
      log(f"Parte inesperada '{part}'; ignorando", level="warning")
