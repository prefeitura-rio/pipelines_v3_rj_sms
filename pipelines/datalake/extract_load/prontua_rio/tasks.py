...
from pipelines.datalake.extract_load.prontua_rio.utils import (
...
@task
def get_file(path, bucket_name, blob_path, environment, blob_type) -> list:
  """
  Faz o download do arquivo do bucket.
  """

  log(f"⬇️ Realizando download de {blob_path}...")
  filename = download_path_from_bucket(
    path=path, bucket_name=bucket_name, blob_prefix=blob_path
  )
...
@task
def unpack_files(
  tar_files: str, output_dir: str, files_to_extract: list, exclude_origin: bool = False
) -> None:
  """
  ...
  """
  log("📁 Iniciando descompactação de arquivo(s)...")
...
def extract_postgres_data(
  ...
  with open(sql_path, "r", encoding="utf-8", errors="ignore", buffering=65536) as f:
    for line in f:
      ...
      if (
        not line or line.startswith("--") or line.startswith("/*") or line.startswith("#")
      ):
        continue
...
def upload_file_to_datalake(
  ...
  try:
    df = pd.read_csv(
      file, sep=",", quotechar='"', skipinitialspace=True, dtype=str, on_bad_lines="warn"
    )
    if not df.empty:
...
@task
def delete_temp_folders(folders):
  for folder in folders:
...
def upload_file_to_native_table(
  ...
  lines = [
    {
      ...
      "loaded_at": datetime.now(tz=pytz.timezone("America/Sao_Paulo"))
      .replace(tzinfo=None)
      .isoformat(),
      "base_type": base_type,
    }
  ...
  try:
    ...
    log(f"Dados problemáticos: {lines[:5]}")
    raise e
