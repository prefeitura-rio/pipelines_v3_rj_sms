...
def report_extraction_status(status: bool, date: str, environment: str = "dev"):
  ...
  log(f"Inserindo em '{FULL_TABLE_NAME}' status sucesso='{success}' em '{date}'...")
  ...
  query_job.result()
  log("Status da extração adicionado na tabela!")


def send_get_request(url: str, type: Optional[str]):
  if type:
    log(f"Fazendo GET esperando resposta '{type}': {url}")
  else:
  ...
  try:
    ...
    if res is not None:
      log(f"Requisição retornou HTTP {res.status_code}", level="error")
    else:
      log(f"Erro ao requisitar URL: {repr(exc)}", level="error")
    return exc
  ...
  if ct not in ALLOWED_CONTENT_TYPES:
    log(f"Content-Type proibido '{ct}' para URL '{url}'", level="warning")
    return None

  if type == "json":
    if ct != "application/json":
      log(f"Esperava Content-Type 'application/json'; recebido '{ct}'", level="warning")
    return res.json()

  if type == "html":
    if ct != "text/html":
      log(f"Esperava Content-Type 'text/html'; recebido '{ct}'", level="warning")
