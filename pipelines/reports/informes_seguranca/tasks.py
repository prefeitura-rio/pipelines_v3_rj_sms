...
def fetch_cids(environment: str, date: str) -> Tuple[list | str, bool]:
  ...
  try:
    ...
    log(f"Erro obtendo dados do BigQuery: {repr(e)}", level="error")
    return (e, True)
...
def build_email(cids: list | str, date: str | None, error: bool) -> Tuple[str, bool]:
  ...
  timestamp = now().strftime("%H:%M:%S de %d/%m/%Y")

  if error:
    log(
      "Flag de erro recebida de task anterior; retornando mensagem de erro",
      level="warning",
    )
    return (
      f"""
  ...
  log(f"Recebidas {len(cids)} ocorrência(s)")

  if len(cids) <= 0:
    log("Nenhuma ocorrência com CIDs especificados", level="warning")
    return (
      f"""
  ...
  for row in cids:
    ...
    if cid in descriptions and cid_descricao != descriptions[cid]:
      warn = f"""
Descrições díspares para CID '{cid}':
\t'{descriptions[cid]}';
\t'{cid_descricao}'
      """.strip()
      ...
      if not descriptions[cid] or len(
        ...
        log(f"{warn}\nEscolhida '{cid_descricao}'", level="warning")
        descriptions[cid] = cid_descricao
      else:
        log(f"{warn}\nEscolhida '{descriptions[cid]}'", level="warning")
    else:
      descriptions[cid] = cid_descricao
...
<table style="font-family:sans-serif;max-width:41em;min-width:19em;color:#13335a">
  ...
  <tr>
    <td>
      <img width="100" align="right" src="{
    informes_seguranca_constants.LOGO_CIT_HORIZONTAL_COLORIDO.value
...
def send_email(
  ...
  if write_to_file_instead:
    log(request_body)
    folder_path = create_tmp_data_folder()
    preview_file_path = f"{folder_path}/infseg.preview.html"
    with open(preview_file_path, "w") as f:
      f.write(message)
    log(f"Arquivo criado em '{preview_file_path}'")
    return
  ...
  response = requests.request(
    "POST", endpoint, headers=request_headers, json=request_body
  )
