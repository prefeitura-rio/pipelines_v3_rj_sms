...
from pipelines.utils.datetime import current_year, now, parse_date_or_today, today
...
WHERE data_publicacao = '{DATE}'
  ...
  for row in rows:
    ...
    do_link = (
      f'<br/><a href="{article_url}">Abrir no D.O.</a>' if len(article_url) > 0 else ""
    )
    ...
    if ERRO_DOU or ERRO_DORJ:
      error_at = (
        ...
        if ERRO_AMBOS
          ...
          "o Diário Oficial da União" if ERRO_DOU else "o Diário Oficial do Município"
        )
      )
      success_at = ""
      if not ERRO_AMBOS:
        success_at = f"""
          <p>
            A extração do {
          "Diário Oficial da União" if not ERRO_DOU else "Diário Oficial do Município"
...
<p style="font-family:sans-serif">
  Email gerado às {now().strftime("%H:%M:%S de %d/%m/%Y")}.
</p>
        """,
      )
...
Email gerado às {now().strftime("%H:%M:%S de %d/%m/%Y")}.
  ...
  subject_edition_string = (
    f"- Edição {CURRENT_VPS_EDITION}ª " if CURRENT_VPS_EDITION else ""
  )
