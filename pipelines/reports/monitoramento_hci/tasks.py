...
def get_data(dataset_name: str, table_name: str, environment: str):
  ...
  project_name = get_google_project_for_environment(environment="prod")
...
GROUP BY 1, 2, 3
  ...
  log(
    f"Consulta por `tipo_evento`, `resultado` ({interval}) retornou {len(rows_recent)} linha(s)"
  )

  QUERY_7D = get_query(interval="7 DAY", denom=denom)
  rows_7d = [row.values() for row in client.query(QUERY_7D).result()]
  log(f"Consulta por `tipo_evento`, `resultado` (7 DAY) retornou {len(rows_7d)} linha(s)")
...
def send_report(data, environment: str):
  ...
  if not IS_WORKDAY:
    ...
    if current_time.minute >= 15 and current_time.minute <= 45:
      log(
        "Não é horário de trabalho; flow configurado para executar a cada 1h. "
        f"Estamos no minuto {current_time.minute}, esperando <15 ou >45"
      )
      return
  ...
  if len(warnings) > 0:
    log(f"Enviando warnings: {warnings}", level="warning")
    message = ""
    if IS_WORKDAY:
